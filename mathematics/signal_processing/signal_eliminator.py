...
def lowpass_filter(data, cutoff=8, fs=100, order=5):
    nyquist = 0.5 * fs
...
def highpass_filter(data, cutoff=17, fs=100, order=5):
    nyquist = 0.5 * fs
    normal_cutoff = cutoff / nyquist
...
def signal_elimination(
        original_signal, low_cutoff: float, high_cutoff: float, fs: int, window_size: int = 10):
    # Step 1: 低周波ノイズの除去
    low_freq_noise = lowpass_filter(original_signal, low_cutoff, fs)
    signal_without_low_freq_noise = original_signal - low_freq_noise

    # Step 2: 高周波ノイズの除去
    high_freq_noise = highpass_filter(signal_without_low_freq_noise, high_cutoff, fs)
    ...
    # Step 4: 差分信号の計算
    difference_signal = calculate_difference_signal(smoothed_signal)

    return smoothed_signal, difference_signal
