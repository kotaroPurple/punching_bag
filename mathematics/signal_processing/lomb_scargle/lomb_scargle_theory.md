...
ドップラーセンサーの IQ 信号のように $x_i = I_i + jQ_i$ が複素値のときは、実数版の

$A\cos(\omega t_i) + B\sin(\omega t_i)$

ではなく、複素指数基底で

$x(t_i) \approx C e^{j\omega t_i}$

を周波数ごとに最小二乗フィットすると扱いやすくなります。

### 6.1 1成分（single complex sinusoid）モデル

各周波数 $f$（$\omega=2\pi f$）で、残差二乗和

$\sum_i \left|x_i - C e^{j\omega t_i}\right|^2$

を最小化する $C$ は閉形式で

$\hat{C}(\omega) = \frac{\sum_i x_i e^{-j\omega t_i}}{\sum_i 1}$

（重みなしの場合）となります。重み $w_i$ を入れると

$\hat{C}(\omega) = \frac{\sum_i w_i x_i e^{-j\omega t_i}}{\sum_i w_i}$

です。

このとき、パワー指標は例えば

$P_c(\omega) = \left|\sum_i x_i e^{-j\omega t_i}\right|^2$

や、その正規化版

$\tilde{P}_c(\omega)=\frac{\left|\sum_i x_i e^{-j\omega t_i}\right|^2}{\sum_i |x_i|^2}$
...
$P_c(+\omega)$ と $P_c(-\omega)$

は一致しません。したがって、周波数グリッドを

$f \in [-f_{\max}, +f_{\max}]$

で探索すれば、正と負の周波数成分を分離して推定できます。

- $+f$ 側ピーク: ある回転方向（位相進み）
- $-f$ 側ピーク: 逆回転方向（位相遅れ）

という解釈が可能です（符号の物理的意味はミキサ定義に依存）。
...
の共役対称を持つため、正負を本質的に区別しにくく、通常は片側スペクトルで十分です。
一方、IQ の複素信号ではこの対称性がないため、両側スペクトルが本質的に必要です。

### 6.4 複素 IQ に対する実務手順

1. 欠損後の不等間隔時刻 $t_i$ と複素観測 $x_i$ を用意する。  
2. DC 成分を除去する（$x_i \leftarrow x_i - \bar{x}$）。  
3. 両側周波数グリッド $[-f_{\max}, +f_{\max}]$ を作る。  
4. 各 $f$ で $\sum_i x_i e^{-j2\pi f t_i}$ を計算し、$|\cdot|^2$ をパワーとする。  
5. 正負それぞれのピーク位置を読む。  

この手順は「複素版 Lomb-Scargle / 不等間隔複素スペクトル推定」とみなせます。
